...
class Transformer_nonautoregressive(FairseqModel):
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs):
        ...
        # execute the decoder
        output_masks = output_tokens.ne(self.decoder.dictionary.pad())
    ...
    def initialize_output_tokens(self, encoder_out, src_tokens):
        # length prediction
        length_tgt = encoder_out['predicted_lengths'].max(-1)[1]
        max_length = length_tgt.clamp_(min=2).max()
        idx_length = utils.new_arange(src_tokens, max_length)

        initial_output_tokens = src_tokens.new_zeros(
            src_tokens.size(0), max_length
        ).fill_(self.decoder.dictionary.pad())
        initial_output_tokens.masked_fill_(
            idx_length[None, :] < length_tgt[:, None], 3
        )
