...
torch::Tensor LevenshteinDistance3(
        torch::Tensor source,
        torch::Tensor target,
        torch::Tensor source_length,
        torch::Tensor target_length) {
    ...
    CHECK_INPUT(source_length);
    CHECK_INPUT(target_length);
    return LevenshteinDistance3Cuda(source, target, source_length, target_length);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("levenshtein_distance3", &LevenshteinDistance3, "Levenshtein distance with subsitutuion");
